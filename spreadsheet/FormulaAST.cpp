...
public:
    ...
    virtual double Evaluate(const std::function<CellInterface::Value(Position)>& args) const = 0;
...
public:
    ...
    double Evaluate(const std::function<CellInterface::Value(Position)>& args) const override
    {
        ...
        default:
            throw std::invalid_argument("operation can't be done");
        }
    }
